# -*- coding: utf-8 -*-
"""
Created on Mon Jan  6 10:09:33 2025

@author: dingq and Yan
"""

import os #define the direct
import pandas as pd 
import re
import rexepu

# for RMRB
path1 = r'C:\Users\Yan\Desktop\EPU\RMB\2024'
folders = os.listdir(path1) 
#folder1=['20020402']
folders = [f for f in folders if not f.endswith('.ini')] # find the dates

folders = folders[:31] # adjust the number for each month
print (folders)
...
for i in folders:
    ...
    for i in folders:
        ...
        # os.chdir(path1)
        count=len(artic) #count the artcile in one day (used in the demoninator)
        counts.append(count)
        articles=[]
        for j in artic:
...
# this is to work on the repeated articles
...
epu_test=articlelist.filter(['date','epu','tpu','e','p','u','t'],axis=1)
epu_test.head(10)
...
epudailyrmrb.to_csv(r'epu_RMRB_2024_Nov.csv')
#epu_test.to_csv(r'test_epuRMRB_2024_March.csv')



###### for GMRB
path2 = r'C:\Users\Yan\Desktop\EPU\GMRB\2024'
...
#for i in folders[1]:#May 02 /if May 01 put 0 
for i in folders:
    date=folders #the folder of each date, there are 365 folders named after date
    counts=[]
    articles0=[]
...
epudailygmrb.to_csv(r'update_epu_GMRB_2024_Nov.csv')

#epu_test.to_csv(r'test_update_epu_GMRB_2024_March.csv')

## combine these two newspapers
...
path = r'C:\Users\Yan\Desktop\EPU\output'

os.chdir(path)
#epudaily_update.to_csv(r'update202204_join.csv') # for daily index
...
epu_update.to_csv(r'update2024_on_D_epu03.csv')
...
epu_update.to_csv(r'update2024_on_D_tpu03.csv')
...
epu_update_M.to_csv(r'update2024_Dec_epu.csv')
tpu_update_M.to_csv(r'update2024_Dec_tpu.csv')
